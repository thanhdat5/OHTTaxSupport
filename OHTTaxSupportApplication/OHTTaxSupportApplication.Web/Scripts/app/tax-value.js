var urlGetAll = "/api/taxvalues/getall";
var urlGetById = "/api/taxvalues/getbyid/";
var urlDelete = "/api/taxvalues/delete/";    
var urlCreate = "/api/taxvalues/create";
var urlUpdate = "/api/taxvalues/update";

function refreshForm() {           
    $('#oht-TaxValue-Value').val(null);
}
...
function loadItemSuccess(data) {
    ...
    if (data.Code != 0) {
        ...
        if (response == null) {
            ...
            // Set value
            $('#oht-TaxValue-ID').val(response.ID);
            $('#oht-TaxValue-Value').val(response.Value);
...
function saveItem() {            
    var id = $('#oht-TaxValue-ID').val();
    var value = $('#oht-TaxValue-Value').val();
    if (value == null || value.trim().length <= 0) {
        ohtToastMessage("error", "Tax Value can not be null.");
        $('#oht-TaxValue-Value').focus();
        return;
    }
    var obj = {
        ID: id,
