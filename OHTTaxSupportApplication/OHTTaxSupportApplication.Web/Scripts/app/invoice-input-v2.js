		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data.Code != 0) {
					...
					$(that).closest('li').before('<li><a id="#invoice_' + id + '_number" href="#invoice_' + id + '">Draft</a> <span id="remove-invoice-' + id + '"><i class="fa fa-trash"></i></span><i class="fa fa-save text-success" id="save-invoice-' + id + '" style="position: absolute;right: 35px;top: 0;z-index: 9;font-size: 18px;padding:9px 10px;cursor:pointer"></i></li>');

					var tabContent = "<div class='tab-pane' id='" + tabId + "'>";
					//###################################################
					tabContent += "<div class='row'>";
					tabContent += "<div class='col-md-3 col-sm-6'>";
					tabContent += "<div class='form-group'><label>Invoice #</label><input type='text' class='form-control' id='txtInvoiceCode-" + id + "' name='txtInvoiceCode-" + id + "' value='" + data.Result.InvoiceCode + "' /><span class='text-danger' id='txtInvoiceCode-" + id + "_error'></span></div>";
					tabContent += "</div></div>";
					...
					tabContent += "<div class='col-md-3 col-sm-6'>";
					tabContent += "<div class='form-group'><label>In/Out&nbsp;<span class='text-danger'>*</span></label><select class='form-control' onchange='inOutChange(" + id + ")' id='cboInOut-" + id + "' name='cboInOut-" + id + "'><option value=''>--Please select--</option><option value='true'>In</option><option value='false'>Out</option></select><span class='text-danger' id='cboInOut-" + id + "_error'></span></div>";
					tabContent += "</div>";

					tabContent += "<div class='hidden-lg hidden-md hidden-sm'><div style='clear:both;width:100%;height:10px;'></div></div>";

					tabContent += "<div class='col-md-3 col-sm-6'>";
					tabContent += "<div class='form-group'><label>Customer&nbsp;<span class='text-danger'>*</span></label><select class='form-control' onchange='customerChange(" + id + ")' id='cboCustomer-" + id + "' name='cboCustomer-" + id + "'>" + customersDDL + "</select><span class='text-danger' id='cboCustomer-" + id + "_error'></span></div>";
					tabContent += "</div>";


					tabContent += "<div class='col-md-3 col-sm-6'>";
					tabContent += "<div class='form-group'><label>Total</label><input disabled='disabled' type='text' class='form-control' id='txtTotal-" + id + "' name='txtTotal-" + id + "' value='0' /><span class='text-danger' id='txtTotal-" + id + "_error'></span></div>";
					tabContent += "</div>";
					tabContent += "</div>";
					//###################################################
					...
					tabContent += "<div class='col-md-4 col-sm-6'>";
					...
					tabContent += "<div class='col-md-2 col-sm-6'>";
					tabContent += "<div class='form-group'><label>Brutto&nbsp;<span class='text-danger'>*</span></label><input type='number' class='form-control' id='txtNET-" + id + "' name='txtNET-" + id + "' /><span class='text-danger' id='txtNET-" + id + "_error'></span></div>";
					// onkeyup='totalNET(" + id + ")' onmouseup='totalNET(" + id + ")'
					tabContent += "</div>";

					tabContent += "<div class='hidden-lg hidden-md hidden-sm'><div style='clear:both;width:100%;height:10px;'></div></div>";

					tabContent += "<div class='col-md-2 col-sm-12'>";
					...
					tabContent += '<thead>';
					tabContent += '<tr>';
					tabContent += '<th class="oht-th-id">In/Out</th>';
...
function saveInvoiceDetails(id) {
	...
	var txtInvoiceCode = $('#txtInvoiceCode-' + id).val();
	...
	if (txtInvoiceCode == null || txtInvoiceCode.trim().length <= 0) {
		$('#txtInvoiceCode-' + id + "_error").text(errRequired);
		isValid = false;
	}
	if (!isValid) {
		return;
	...
	var postModel = {
		Date: txtDateCreated,
		CustomerID: parseInt(cboCustomer),
		CustomerName: cboCustomerName,
		DepartmentName: cboDepartmentName,
		CategoryName: cboCategoryName,
		TaxValueValue: cboTaxValueValue,
		InvoiceCode: txtInvoiceCode,
		TaxValue: parseInt(cboTaxValue),
		ID: 0,
		InOut: cboInOut.toString() == 'true' ? true : false,
		InvoiceID: id,
		Value: Number(txtNET),
		DepartmentID: parseInt(cboDepartment),
		CategoryID: parseInt(cboCategory),
		IsActive: true
	};
...
function resetFormDetail(id) {
	...
	$('#cboCategory-' + id).val("");
	$('#txtNET-' + id).val("0");
	$('#cboDepartment-' + id)[0].selectedIndex = 1;
}
		...
		if (invoicesList[i].id == invoiceID) {
			...
			for (var j = 0; j < invoicesList[i].details.length; j++) {
				if (invoicesList[i].details[j].ID == id) {
					...
					$('#cboInOut-' + invoiceID).val(detail.InOut.toString());
					...
					$('#txtInvoiceCode-' + invoiceID).val(detail.InvoiceCode);
				}
			}
		}
	}
...
function inOutChange(id) {
	for (var i = 0; i < invoicesList.length; i++) {
		if (invoicesList[i].id == id) {
			if (invoicesList[i].details.length > 0) {
				var cboInOut = $('#cboInOut-' + id).val();

				for (var j = 0; j < invoicesList[i].details.length; j++) {
					invoicesList[i].details[j].InOut = cboInOut;
				}
			}
		}
		refreshDataDetail(id);
	...
	for (var i = 0; i < invoicesList.length; i++) {
		if (invoicesList[i].id == id) {
			if (invoicesList[i].details.length > 0) {
				...
				for (var j = 0; j < invoicesList[i].details.length; j++) {
					invoicesList[i].details[j].CustomerID = cboCustomer;
					invoicesList[i].details[j].CustomerName = cboCustomerName;
				}
			}
		}
		refreshDataDetail(id);
...
function refreshDataDetail(id) {
	...
	totalNET(id);
	if (invoiceIndex == -1) {
		$('#oht-invoice-data-' + id).html('<tr class="oht-loading"><td colspan="9">No record found.</td></tr>');
	...
	if (invoicesList[invoiceIndex].details == null || invoicesList[invoiceIndex].details.length <= 0) {
		...
		$.each(invoicesList[invoiceIndex].details, function (index, element) {
			innerHtml +=
				'<tr>' +
				'<td>' + ((element.InOut == "true" || element.InOut == true) ? "In" : "Out") + '</td>' +
				'<td>' + element.CustomerName + '</td>' +
				'<td>' + element.DepartmentName + '</td>' +
				'<td>' + element.CategoryName + '</td>' +
				'<td>' + element.Value + ' Brutto </td>' +
